...
convertScanHedder:: [String] -> [String]
convertScanHedder y = analyzeScanHedder
    where x = concat y
          analyzeScanHedder = [ls] ++ [ns] ++ component ++ [ss] ++ [se] ++ [ah] ++ [ai]
          ls = concat ([byteLoder 2 x] ++ [byteLoder 3 x])
          ns = (byteLoder 4 x)
          ns' = hex2dec ns
...
main = do
    ...
    let binaryList = binaryListMaker binaryData
        ...
        imageData = markerFilter "ff00" binaryList
        scanHedder = markerFilter "ffda" binaryList
        huffmanTable = convertHuffmanTable $ markerFilter "ffc4" binaryList
    print (convertScanHedder scanHedder)
    let test = (huffmanTable)
    saveFile <- openFile "test4.txt" WriteMode
    hPrint saveFile binaryData
