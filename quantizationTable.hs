...
convertQuantization :: [String] -> [[Int]]
convertQuantization (x:xs) = map hex2dec([lq] ++ concat dqt) : convertQuantization xs
    where lq = (byteLoder 2 x) ++ (byteLoder 3 x)
          dqt = tableReader(drop 8 x)
          tableReader :: String -> [[String]]
          tableReader (y1:y2:ys) =  ([[[y1]] ++ [[y2]]]) ++ (quantamReader 64 ys) : tableReader (drop 128 ys)
          tableReader y = []
          quantamReader :: Int -> String -> [String]
          quantamReader 0 _ = []
          quantamReader n (z1:z2:zs) = ([z1] ++ [z2]) :quantamReader (n-1) zs
convertQuantization x = []
...
main = do
    ...
    let binaryList = binaryListMaker binaryData
        ...
        quantizationTable = convertQuantization $ markerFilter "ffdb" binaryList
